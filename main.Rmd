...
On Hydroshare you have the option to either select all *.7z* files and download them in a zipped folder or download a custom selection of files if your are only interested in a specific region (area of interest) or specific hydrologic orders. For creating a user defined selection you can use the search bar to filter the files for a spatial coverage or a hydrologic order. If you want to check more precisely whether your area of interest is covered by this dataset at all or which files are relevant, please see [this interactive map](https://mxnl.github.io/macro_mohp_feature/).
