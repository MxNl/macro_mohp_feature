...
Some of the GeoTiff files representing DSD ("*\<abbreviation of the EU-MOHP measure\> = dsd*") contain defective data for a currently unknown reasons. This does not affect files for the spatial coverage of *europemainland* ("*\<region name for spatial coverage\> = europemainland*"). You can recognize these defective files by the values of the raster grid cells (-1,79769e+308 and 1,79769e+308).

We also note that a file was created for each spatial coverage and for each hydrologic order regardless of the existence of a river network for a hydrologic order for a given spatial coverage. These files must be disregarded depending on the use case. 
...
8.  Before running the pipeline on the full spatial coverage of the EEA39 countries, we recommend to test the pipeline with the smaller test study area by setting the variable *area* in the file *config.yml* to "test". If the pipeline works, you can change it back to "europe".
