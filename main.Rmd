...
After downloading the dataset from the mentioned link in [Data Records], the dataset must be unzipped. Depending on the location of the area of interest (AOI) and the desired hydrologic orders, the relevant files can be selected from the sub-folders. If you want to check more precisely whether your AOI is covered by this dataset and which files are relevant, please see this [interactive map](https://mxnl.github.io/macro_mohp_feature/).
...
```{r dfddstatsbarplot, echo=FALSE, out.width = "80%", fig.asp=0.5, fig.showtext = TRUE, fig.cap= 'Distribution of the values in the attribute column dfdd. BH140 (= river) is by far the most frequent value.'}
