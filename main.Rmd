...
Subsequently, a column `levelpath_id` was added as a unique ID for all geometries belonging to the same mainstem. The geometries of the respective river network was then merged by using this column. This results in a river network for each hydrologic order separately with a reduced number of geometries as multiple geometries are now summarised into mainstems.
...
```{r mainstemscheme, echo=FALSE, out.width = "100%", fig.showtext = TRUE, fig.cap= 'Schematic river network before (A) and after (B) merging linestring geometries.'}
